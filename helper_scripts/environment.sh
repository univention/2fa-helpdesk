...
if [ -z "$NAMESPACE" ]; then
    read -p "Enter the namespace (or do export NAMESPACE= yourself): " NAMESPACE
fi
...
UCS_ADMIN_PASS=$(kubectl -n yschmidt-opendesk get secrets ums-nubus-credentials -o jsonpath="{.data.administrator_password}" | base64 -d)

# TODO: is this the correct one?
# get guardian management API secret #
UCS_GUARDIAN_MGMT_SECRET=$(kubectl -n yschmidt-opendesk get secrets ums-opendesk-guardian-client-secret -o jsonpath="{.data.managementApiClientSecret}" | base64 -d)
